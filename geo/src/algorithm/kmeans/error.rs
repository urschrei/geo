...
pub enum KMeansInitError {
    /// Input data contains NaN (Not-a-Number) coordinates
    ///
    ...
    WeightedDistributionFailed {
        /// Description of the underlying failure from rand's WeightedIndex
        reason: String,
    },
}
...
impl std::fmt::Display for KMeansInitError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            KMeansInitError::WeightedDistributionFailed { reason } => {
                write!(
                    f,
                    "Failed to create weighted distribution for k-means++ sampling: {}",
                    reason
